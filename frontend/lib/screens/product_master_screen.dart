...
class _ProductMasterScreenState extends State<ProductMasterScreen> {
  ...
  String _selectedStudded = 'N'; // 'Y' or 'N'
  String _selectedDiaStone = ''; // 'D' (Diamond), 'S' (Stone), 'P' (Precious), or ''
  String _selectedStoneUnit = 'CARAT'; // 'CARAT' (CTS) or 'GRAM' (GMS)
  ...
  void _applyFilter(String query) {
    ...
    if (_searchQuery.isEmpty) {
      ...
      _filteredProducts = _products.where((p) {
        final diastoneLabel = p.diastone == 'D'
            ? 'diamond'
            : (p.diastone == 'S' ? 'stone' : (p.diastone == 'P' ? 'precious' : 'plain'));
        return p.productname.toLowerCase().contains(_searchQuery) ||
            p.productid.toString().contains(_searchQuery) ||
            p.hsncode.toLowerCase().contains(_searchQuery) ||
            ...
            p.studded.toLowerCase().contains(_searchQuery) ||
            p.diastone.toLowerCase().contains(_searchQuery) ||
            diastoneLabel.contains(_searchQuery) ||
            p.stoneunit.toLowerCase().contains(_searchQuery);
      }).toList();
    }
  }
  ...
  void _openForm([ProductRecord? existing]) {
    setState(() {
      ...
      if (existing != null) {
        ...
        _hsnCodeController.text = existing.hsncode;
        ...
        _selectedStudded = existing.studded.isNotEmpty ? existing.studded : (existing.havestoneDiamond == 'YES' ? 'Y' : 'N');
        _selectedDiaStone = existing.diastone;
        _selectedStoneUnit = existing.stoneunit.isNotEmpty ? existing.stoneunit : 'CARAT';
      } else {
        _resetFormFields();
      }
    });
  ...
  Future<void> _saveProductForm() async {
    ...
    try {
      final isStudded = _selectedStudded == 'Y' || _selectedDiaStone.isNotEmpty;
      final record = ProductRecord(
        ...
        havestoneDiamond: isStudded ? 'YES' : _selectedHaveStone,
        havesubproduct: _selectedHaveSubProduct,
        studded: isStudded ? 'Y' : 'N',
        diastone: _selectedDiaStone,
        hsncode: _hsnCodeController.text.trim(),
        stoneunit: isStudded ? _selectedStoneUnit : '',
      );
      ...
      child: Form(
        ...
        child: Column(
          ...
          children: [
            ...
            // Row 1: Product ID (Autogenerated), Category Name, Product Name, and HSN Code
            Wrap(
              spacing: 16,
              runSpacing: 14,
              children: [
                  ...
                  child: Column(
                    ...
                    children: [
                      ...
                      TextFormField(
                        ...
                        decoration: _inputDecoration("e.g. Mens Diamond Ring, Gold Necklace").copyWith(
                          counterText: "",
                        ),
                        validator: (val) {
                  ...
                  child: Column(
                    ...
                    children: [
                      ...
                      TextFormField(
                        ...
                        maxLength: 12,
                        style: const TextStyle(color: GlassTheme.textPrimary, fontSize: 14, fontWeight: FontWeight.w700),
                        decoration: _inputDecoration("e.g. 7113, 7102").copyWith(
                          counterText: "",
                          prefixIcon: const Icon(Icons.tag_rounded, size: 18, color: Color(0xFF8B5CF6)),
                        ),
                      ),
                    ],
                  ),
            ...
            // Row 2: Calculation Type, Stock Type, Have Sub Product
            const Text(
              "Billing & Stock Tracking Rules",
                  ...
                  child: Column(
                    ...
                    children: [
                      ...
                      DropdownButtonFormField<String>(
                        ...
                        items: const [
                          DropdownMenuItem(value: 'SKU', child: Text("SKU (Barcode Tagged Item)", style: TextStyle(color: GlassTheme.textPrimary, fontWeight: FontWeight.w700))),
                          DropdownMenuItem(value: 'OPEN', child: Text("OPEN (Bulk Weight Inventory)", style: TextStyle(color: GlassTheme.textPrimary, fontWeight: FontWeight.w700))),
                        ],
                        onChanged: (val) {
                  ...
                  child: Column(
                    ...
                    children: [
                      ...
                      DropdownButtonFormField<String>(
                        ...
                        items: const [
                          DropdownMenuItem(value: 'YES', child: Text("YES (Has Parts / Subproducts)", style: TextStyle(color: Color(0xFF3B82F6), fontWeight: FontWeight.w800))),
            ...
            // Row 3: Studded & Diamond/Stone Master Specifications
            Container(
                  ...
                  Row(
                    children: [
                      Container(
                        ...
                        child: const Icon(Icons.diamond_rounded, color: Color(0xFFD97706), size: 18),
                      ),
                      const SizedBox(width: 8),
                      const Text(
                        "Studded Stone & Diamond Classification",
                        style: TextStyle(color: GlassTheme.textPrimary, fontSize: 14, fontWeight: FontWeight.w800),
                      ),
                      const Spacer(),
                      if (_selectedStudded == 'Y' || _selectedDiaStone.isNotEmpty)
                        Container(
                          ...
                          child: const Text(
                            "Studded / Stone Active",
                            style: TextStyle(color: Color(0xFFB45309), fontSize: 11, fontWeight: FontWeight.w800),
                          ),
                        ),
                    ],
                  ),
                    ...
                    children: [
                      ...
                      SizedBox(
                        ...
                        child: Column(
                          ...
                          children: [
                            const Text(
                              "Studded Product (studded = Y/N) *",
                              style: TextStyle(color: GlassTheme.textPrimary, fontSize: 12, fontWeight: FontWeight.w700),
                            ),
                            const SizedBox(height: 6),
                            DropdownButtonFormField<String>(
                              value: _selectedStudded,
                              dropdownColor: Colors.white,
                              style: const TextStyle(color: GlassTheme.textPrimary, fontSize: 13, fontWeight: FontWeight.w700),
                              decoration: _inputDecoration("Studded?"),
                              items: const [
                                DropdownMenuItem(
                                  value: 'N',
                                  child: Row(
                                    children: [
                                      Icon(Icons.remove_circle_outline_rounded, color: GlassTheme.textMuted, size: 16),
                                      SizedBox(width: 6),
                                      Text("N - Plain Product (No Stones)", style: TextStyle(color: GlassTheme.textPrimary, fontWeight: FontWeight.w700)),
                                    ],
                                  ),
                                ),
                                DropdownMenuItem(
                                  ...
                                  child: Row(
                                    children: [
                                      Icon(Icons.diamond_outlined, color: Color(0xFFD97706), size: 16),
                                      SizedBox(width: 6),
                                      Text("Y - Studded / Stone Product", style: TextStyle(color: Color(0xFFD97706), fontWeight: FontWeight.w800)),
                                    ],
                                  ),
                                ),
                              ],
                              onChanged: (val) {
                                if (val != null) {
                                  setState(() {
                                    _selectedStudded = val;
                                    if (val == 'Y') {
                                      _selectedHaveStone = 'YES';
                                      if (_selectedDiaStone.isEmpty) _selectedDiaStone = 'S';
                                    } else {
                                      _selectedHaveStone = 'NO';
                                      _selectedDiaStone = '';
                                    }
                      ...
                      // 2. DIASTONE = D (Diamond) / S (Stone) / P (Precious)
                      SizedBox(
                        width: isMobile ? double.infinity : 300,
                        child: Column(
                          crossAxisAlignment: CrossAxisAlignment.start,
                          children: [
                            const Text(
                              "Stone Type (DIASTONE = D / S / P)",
                              style: TextStyle(color: GlassTheme.textPrimary, fontSize: 12, fontWeight: FontWeight.w700),
                            ),
                            const SizedBox(height: 6),
                            DropdownButtonFormField<String>(
                              value: _selectedDiaStone,
                              dropdownColor: Colors.white,
                              style: const TextStyle(color: GlassTheme.textPrimary, fontSize: 13, fontWeight: FontWeight.w700),
                              decoration: _inputDecoration("Diamond / Stone"),
                              items: const [
                                DropdownMenuItem(
                                  value: '',
                                  child: Text("None (Plain Metal)", style: TextStyle(color: GlassTheme.textMuted, fontWeight: FontWeight.w600)),
                                ),
                                DropdownMenuItem(
                                  value: 'D',
                                  child: Row(
                                    children: [
                                      Icon(Icons.diamond_rounded, color: Color(0xFF0284C7), size: 16),
                                      SizedBox(width: 6),
                                      Text("D - Diamond", style: TextStyle(color: Color(0xFF0284C7), fontWeight: FontWeight.w800)),
                                    ],
                                  ),
                                ),
                                DropdownMenuItem(
                                  ...
                                  child: Row(
                                    children: [
                                      Icon(Icons.blur_on_rounded, color: Color(0xFF059669), size: 16),
                                      SizedBox(width: 6),
                                      Text("S - Stone (Synthetic / Semi-Precious)", style: TextStyle(color: Color(0xFF059669), fontWeight: FontWeight.w800)),
                                    ],
                                  ),
                                ),
                                DropdownMenuItem(
                                  ...
                                  child: Row(
                                    children: [
                                      Icon(Icons.auto_awesome_rounded, color: Color(0xFFDC2626), size: 16),
                                      SizedBox(width: 6),
                                      Text("P - Precious Stone (Ruby, Emerald, etc.)", style: TextStyle(color: Color(0xFFDC2626), fontWeight: FontWeight.w800)),
                                    ],
                                  ),
                                ),
                              ],
                      ...
                      SizedBox(
                        ...
                        child: Column(
                          ...
                          children: [
                            const Text(
                              "Stone Unit (stoneunit: Carat / Gram)",
                              style: TextStyle(color: GlassTheme.textPrimary, fontSize: 12, fontWeight: FontWeight.w700),
                            ),
                            const SizedBox(height: 6),
                            DropdownButtonFormField<String>(
                              ...
                              dropdownColor: Colors.white,
                              style: const TextStyle(color: GlassTheme.textPrimary, fontSize: 13, fontWeight: FontWeight.w700),
                              decoration: _inputDecoration("Stone Unit"),
                              items: const [
                                DropdownMenuItem(
                                  value: 'CARAT',
                                  child: Row(
                                    children: [
                                      Icon(Icons.scale_rounded, color: Color(0xFF8B5CF6), size: 16),
                                      SizedBox(width: 6),
                                      Text("CARAT (CTS - 1 Cts = 0.200g)", style: TextStyle(color: GlassTheme.textPrimary, fontWeight: FontWeight.w800)),
                                    ],
                                  ),
                                ),
                                DropdownMenuItem(
                                  ...
                                  child: Row(
                                    children: [
                                      Icon(Icons.balance_rounded, color: Color(0xFF10B981), size: 16),
                                      SizedBox(width: 6),
                                      Text("GRAM (GMS - Direct Grams)", style: TextStyle(color: GlassTheme.textPrimary, fontWeight: FontWeight.w800)),
                                    ],
                                  ),
                                ),
                              ],
                              onChanged: (_selectedStudded == 'Y' || _selectedDiaStone.isNotEmpty)
                                  ? (val) {
                                      if (val != null) setState(() => _selectedStoneUnit = val);
                                    }
                                  : null,
                            ),
                          ],
              ...
              child: Container(
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    if (prod.hsncode.isNotEmpty) ...[
                      const SizedBox(height: 6),
                      _buildInfoRow("HSN Code", prod.hsncode),
                    ],
                    const SizedBox(height: 6),
                    ...
                    Row(
                      mainAxisAlignment: MainAxisAlignment.spaceBetween,
                      children: [
                        const Text("Studded / Stone", style: TextStyle(fontSize: 12, color: GlassTheme.textSecondary, fontWeight: FontWeight.w600)),
                        Container(
                          padding: const EdgeInsets.symmetric(horizontal: 8, vertical: 2),
                          decoration: BoxDecoration(
                            color: (prod.studded == 'Y' ? const Color(0xFFD97706) : const Color(0xFF64748B)).withValues(alpha: 0.12),
                            borderRadius: BorderRadius.circular(6),
                          ),
                          child: Text(
                            prod.studded == 'Y' ? "YES (Studded)" : "NO (Plain)",
                            style: TextStyle(
                              fontSize: 11,
                              fontWeight: FontWeight.w800,
                              color: prod.studded == 'Y' ? const Color(0xFFD97706) : const Color(0xFF64748B),
                            ),
                          ),
                        ),
                      ],
                    ),
                    if (prod.diastone.isNotEmpty) ...[
                      const SizedBox(height: 6),
                      _buildInfoRow(
                        "Stone Type",
                        prod.diastone == 'D'
                            ? "D - Diamond"
                            : (prod.diastone == 'S' ? "S - Synthetic/Stone" : (prod.diastone == 'P' ? "P - Precious Stone" : prod.diastone)),
                      ),
                    ],
                    if (prod.stoneunit.isNotEmpty) ...[
                      const SizedBox(height: 6),
                      _buildInfoRow("Stone Unit", prod.stoneunit == 'CARAT' ? "CARAT (CTS)" : "GRAM (GMS)"),
                    ],
                    const SizedBox(height: 6),
        ...
        child: SingleChildScrollView(
          ...
          child: DataTable(
            ...
            columns: const [
              ...
              DataColumn(label: Text("HSN CODE", style: TextStyle(color: GlassTheme.textPrimary, fontWeight: FontWeight.w800, fontSize: 12))),
              DataColumn(label: Text("STUDDED", style: TextStyle(color: GlassTheme.textPrimary, fontWeight: FontWeight.w800, fontSize: 12))),
              DataColumn(label: Text("DIA/STONE", style: TextStyle(color: GlassTheme.textPrimary, fontWeight: FontWeight.w800, fontSize: 12))),
              DataColumn(label: Text("STONE UNIT", style: TextStyle(color: GlassTheme.textPrimary, fontWeight: FontWeight.w800, fontSize: 12))),
            ...
            rows: _filteredProducts.map((prod) {
              final isStudded = prod.studded == 'Y';
              String diaLabel = '-';
              Color diaColor = GlassTheme.textMuted;
              if (prod.diastone == 'D') {
                diaLabel = 'D (Diamond)';
                diaColor = const Color(0xFF0284C7);
              } else if (prod.diastone == 'S') {
                diaLabel = 'S (Stone)';
                diaColor = const Color(0xFF059669);
              } else if (prod.diastone == 'P') {
                diaLabel = 'P (Precious)';
                diaColor = const Color(0xFFDC2626);
              }
              ...
              return DataRow(
                cells: [
                  ...
                  DataCell(Text(prod.hsncode.isNotEmpty ? prod.hsncode : '-', style: const TextStyle(color: GlassTheme.textSecondary, fontWeight: FontWeight.w700, fontSize: 12))),
                  DataCell(
                    Container(
                      padding: const EdgeInsets.symmetric(horizontal: 8, vertical: 2),
                      decoration: BoxDecoration(
                        color: (isStudded ? const Color(0xFFD97706) : const Color(0xFF64748B)).withValues(alpha: 0.12),
                        borderRadius: BorderRadius.circular(6),
                      ),
                      child: Text(
                        isStudded ? "Y" : "N",
                        style: TextStyle(
                          color: isStudded ? const Color(0xFFD97706) : const Color(0xFF64748B),
                          fontWeight: FontWeight.w800,
                          fontSize: 11,
                        ),
                      ),
                  ...
                  DataCell(
                    Text(
                      prod.stoneunit.isNotEmpty ? (prod.stoneunit == 'CARAT' ? 'Carat (Cts)' : 'Gram (Gms)') : '-',
